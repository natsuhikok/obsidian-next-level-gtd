...
export class NextActionsView extends ItemView {
	...
	constructor(
		...
		super(leaf);
		this.selection = FilterSelection.initial(plugin.settings.environmentContexts);
	}
	...
	private render() {
		...
		const query = new NextActionsQuery(
			this.selection,
			this.plugin.settings.environmentContexts,
			allActions,
			today,
		);
		...
		if (allPropertyCandidates.length > 0) {
			this.renderPropertyBlock(
				...
				new Set(query.propertyCandidates),
		...
		const sorted = query.sortedFilteredActions;
		...
		sorted.forEach((action) => {
			...
			this.renderContextBadges(body, action, query.classifier);
	...
	private renderEnvBlock(filterBar: HTMLElement) {
		const envContexts = this.selection.environmentContexts;
		const row = filterBar.createDiv({ cls: 'gtd-context-filter-row' });

		const addEnvChip = (label: string, value: string) => {
			const active =
				value === '__all__'
					? this.selection.isAllEnvironmentsSelected
			...
			chip.addEventListener('click', () => {
				if (value === '__all__') {
					this.selection = this.selection.withAllEnvironmentsToggled();
